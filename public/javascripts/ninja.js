...
var Ninja = function(options) {
  ...
  this.Block = function(options) {
    ...
    this.ConfirmActivation = function(callback) {
      ...
      confirmXHR.setRequestHeader('Accept', 'application/json');
     ...
     ***************/
     ...
     this.GetModules = function(callback) {
      ...
      var url = this.ServerUrl() + '/module/block/' + this.Options.nodeId + ninja.Authentication.getAuthSlug();;
  ...
  this.Module = function(options) {
    ...
    this.GetConfig = function(callback) {
      var url = this.ServerUrl() + '/module/' + this.Options.name + '/block/' + this.Block.Options.nodeId + ninja.Authentication.getAuthSlug();;

      var xhr = Ninja.Utilities.CreateXHR(function(error, response) {
        if (error) {
          if (ninja.Options.debug) console.log("Error", error);

        } else {
          if (ninja.Options.debug) console.log("Response", response);
          if (callback) {
            callback(response);
          }
        }
  ...
  this.Device = function(options) {
    ...
    this.Emit = function(data, callback) {
      ...
      if (!this.Options.block.Options.token) {
        ...
        if (ninja.Options.debug) console.log("Emit (PUT): ", this.GUID(), this.Options.block.Options.server);
        var xhr = Ninja.Utilities.CreateXHR(function(error, response) {
          if (callback) {
        ...
        var url = this.ServerUrl() + '/block/' + this.Options.block.Options.nodeId + '/data';
        ...
        if (ninja.Options.debug) console.log("Emit (POST): ", this.GUID(), this.Options.block.Options.server);
        var xhr = Ninja.Utilities.CreateXHR(function(error, response) {
          if (callback) {
            callback(error, response);
          }
